...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=pulmer75/complex-client
kubectl set image deployments/server-deployment server=pulmer75/complex-server
kubectl set image deployments/worker-deployment worker=pulmer75/complex-worker
