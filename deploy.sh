...
kubectl apply -f k8s
kubectl set image deployments/client-deployment client=pulmer75/complex-client:$APP_VERSION
kubectl set image deployments/server-deployment server=pulmer75/complex-server:$APP_VERSION
kubectl set image deployments/worker-deployment worker=pulmer75/complex-worker:$APP_VERSION
